...
@objc(StripeSdk)
class StripeSdk: RCTEventEmitter, STPApplePayContextDelegate, STPBankSelectionViewControllerDelegate, UIAdaptivePresentationControllerDelegate, STPPaymentContextDelegate {
    ...
    @objc(initCustomerContext)
    func initCustomerContext() -> Void {
        self.sendEvent(withName: "onDidSetShippingMethod", body: "icc")
    ...
    func showPaymentOptionsModal() -> Void {
        DispatchQueue.main.async {
            ...
            let paymentContext = STPPaymentContext.init(customerContext: self.customerContext!, configuration: paymentConfig, theme: STPTheme.defaultTheme)

            paymentContext.delegate = self as STPPaymentContextDelegate
    ...
    @objc(getPaymentMethodId:rejecter:)
    func getPaymentMethodId(resolve: @escaping RCTPromiseResolveBlock,
